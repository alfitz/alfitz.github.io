(() => {
    ...
    const drawChart = (chartData) => {
        ...
        const x = d3.scaleTime()
            .domain([new Date(1988, 0, 1), new Date(2003, 0, 1)]) // how many inputs are there?
            .range([0, width]); // what values can the input have?

        const y = d3.scaleLinear()
            .domain([d3.max(chartData, d => d.popularity), 40]).nice()
        ...
        const rectangle = svg.append("rect")
            ...
            .attr("y", 0 + margin.top)
            .attr("width", x(new Date(1991, 0, 1))) // how does this work?
